...
| Задача | qwen2.5-coder:7b | Оценка | Детали |
|--------|-----------------|--------|--------|
| T01 (HttpTimeout) | Частично ✓/✗ | 3/5 | Правильный import + install(HttpTimeout), но не добавляет `jsonConfig` — не показывает полный файл |
| T04 (CommonJson) | ✓ | 5/5 | Точный синтаксис, правильный package, правильный import |
| T08 (Test Day34) | Не компилируется ✗ | 1/5 | Использует `org.junit.Before` (не KMP), `Day34ViewModel.State` (вложенный тип не так виден), `stateFlow.collectLatest` в тесте — зависает навсегда, `MutableStateFlow` как внешний state — неверная архитектура |

### Анализ qwen2.5-coder:7b

**Сильные стороны:**
- Простые задачи "добавь import + блок кода" — справляется хорошо (T01, T04)
- Синтаксис Kotlin корректный для простых файлов
- CommonJson.kt: идеальный результат (5/5)

**Слабые стороны:**
- Тесты (T08): не знает паттерн kotlin.test для KMP. Генерирует JUnit (@Before, org.junit.Test), не kotlin.test
- Архитектурные ошибки: `Day34ViewModel.State`, `Day34ViewModel.Event.InputChanged` — неверные пути к вложенным типам (они не nested, а top-level в пакете)
- `stateFlow.collectLatest` в тесте = бесконечное ожидание, тест зависнет
- Нет понимания KMP-специфики (нет `runTest`, нет `Dispatchers.setMain`)
- Сломается при попытке скомпилировать

### Итог сравнения

| Критерий | claude-sonnet-4-6 | qwen2.5-coder:7b |
|----------|-------------------|-----------------|
| Задач выполнено | 14/18 (78%) | ~1.3/3 (~43%) |
| Компилируется с первого раза | 100% | ~33% |
| Понимание KMP/MVI | Отличное | Слабое |
| Агентный режим (чтение файлов) | Да | Нет |
| Скорость | ~15-30 сек/задача | ~3-8 сек/задача |
| Стоимость | ~$0.003-0.01/задача | $0 |
| Подходит для loop | Да | Нет (тесты ломаются) |
